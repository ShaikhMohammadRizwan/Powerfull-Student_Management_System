...
def add_student(request):
    if request.method == 'POST':
        ...
        # Validation for present_address (if it should not be empty)
        if not present_address:
            messages.error(request, "Parent address is required.")
            return render(request, 'students/add-student.html')

        # Validation for father and mother contact
        if not father_contact:
            messages.error(request, "Father's contact is required.")
            return render(request, 'students/add-student.html')

        if not mother_contact:
            messages.error(request, "Mother's contact is required.")
            return render(request, 'students/add-student.html')

        # Save Parent Information first
        ...
        # Now Save Student Information and associate it with Parent
        student = Student.objects.create(
            ...
            parent=parent  # Associate parent with the student
        )
...
def view_student(request, slug):
    student = get_object_or_404(Student, student_id=slug)  # Ensure the slug matches your field
    context = {
        'student': student,
    }
